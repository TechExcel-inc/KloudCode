...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(Brand.project)) {
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes(`# ${Brand.app}`) || content.includes(`${Brand.project}/bin`)) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(`${Brand.project}/bin`) || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(`${Brand.project}/bin`)) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(Brand.project))
    ) {
      continue
    }
