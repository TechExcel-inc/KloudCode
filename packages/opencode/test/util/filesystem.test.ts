...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "kloudcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "kloudcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "kloudcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "kloudcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["kloudcode.json", "kloudcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "kloudcode.json"),
        path.join(tmp.path, "kloudcode.jsonc"),
        path.join(project, "kloudcode.json"),
        path.join(project, "kloudcode.jsonc"),
      ])
    })
  })
